...
החזר JSON בלבד:
{
  ...
  "verify_all_populated": false,            // true רק ל"ודא שכל השדות מאוכלסים"
  "verify": [                               // שדות לאימות ביעד — **בלי** ערך צפוי (המערכת מחשבת אותו דטרמיניסטית)
    {"target_field": "<שם>"},
...
כללים (קריטי):
- ★★★★ **חשב ערכים סופיים מהדוגמה.** דוגמאות: "ת"ז מתחילה ב-2/5" → קרא את הערך הנוכחי בשדה (למשל
  `0999735863`) והחזר אותו עם התו הראשון מוחלף (`2999735863`) — **שמור אורך ושאר ספרות**, אל תפברק.
  "שדה X עם ערכים משורשרים/מרובים" → קרא את הערך הנוכחי ובנה רשימה/ערך עם **2+ פריטים** (למשל הקיים + עוד אחד).
  "קוד בדיקה = M_PAT_NGC" → value="M_PAT_NGC" (ערך-המקור, לא ערך-היעד).
- ★ **אל תחשב ערכי-יעד צפויים** — רק שמות-שדה ב-verify; המערכת מחשבת את הערך הצפוי מהמקור+החוק (דטרמיניסטי).
- ★ override רק לשדה עם **מקור-יחיד** הניתן לדריסה. שדה מחושב מכמה מקורות (שם מלא = שם-פרטי + משפחה) → רק verify.
  בספק — **קבע (override), אל תימנע**.
- ★ תרחיש שלילי (ערך-לא-תקין/צה"ל) → override עם הערך + expect_no_message=true.
- ★ "האובייקט X לא נבנה" → verify expect:"absent"; "אל תשלח שדה Y" → override op:"remove" על Y.
- ★ אם התסריט דורש אובייקט/role שאין בדוגמה (רופא מפנה code=R כשיש רק מבצע) → פשוט verify על השדה; המערכת
  תבנה/תמיר את ה-resource לבד. אל תנסה לשחזר אותו.
- ★ אמת רק את השדות שהתסריט **באמת** מבקש (תסריט-Header → שדות-Header בלבד, לא referral/organ לא-קשורים).
- ★★★ השתמש **רק** בשמות מ-AVAILABLE_FIELDS. שמות לוגיים בלבד — בלי נתיבים/אינדקסים/מבני-JSON. JSON תקני בלבד.
"""
