...
כללים:
- ★★★★ **overrides רק לשדות שהתסריט אומר במפורש לשנות/לשלוח במקור** (קוד בדיקה, ת"ז לא-תקינה, ערך שגוי).
  שדות ש**מחושבים/מחולצים ע"י ה-Worker** (member_name=שרשור, member_id/member_id_code=פיצול, institute,
  referral_practitioner/act_practitioner=lookup) — **אף פעם לא ב-overrides!** הם לאימות בלבד (verify).
  override על שדה מחושב משחית את המסר. אם בספק — שים ב-verify, לא ב-overrides.
    ...
    def _parse_anchored_response(self, test_case_id, ado_id, text, data) -> Optional[DotNetExecutableTestCase]:
        ...
        for ov in (data.get("overrides") or []):
            ...
            if not src:
                ...
                continue
            # ★ שדה נגזר/מחושב (concatenation 'a + b', FIXED/DERIVED, lookup) — אין לו נתיב-מקור יחיד
            # שאפשר לדרוס; override עליו משחית את המסר (למשל name→"0"). מדלגים — שדות כאלה לאימות בלבד.
            if not _is_concrete_source_path(src):
                notes.append(f"override '{tf}' מצביע על שדה נגזר/מחושב ({src}) — אין override; אימות בלבד. דולג")
                continue
            if ov.get("op") == "remove":
